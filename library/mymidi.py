...
##################
"""
extract SYNTH, SOUNDFONT and SUB_CONFIG_FILE from parser.cfg
"""

def getField(df, pattern):
    df = df[df["class"].str.match(f'{pattern}[*]')]
    if len(df) != 1:
        print(f"Config file: {configFile} must have exactly 1 active {pattern}")
        sys.exit()
    synth = df["opt"].to_string(index=False).strip()
    return synth

def getConfigInfo(configFile):
    try:
        names = "class opt".split()
        df = pd.read_csv(configFile, names=names, sep=r'\s+')
    except:
        print(f"Config file:{configFile} is missing")
        sys.exit()
    print(df)
    return df

#os.chdir("/Users/seddon/play/mymusic")
configFile = "parser.cfg"
df = getConfigInfo(configFile)
SYNTH = getField(df, "SYNTH")
SOUNDFONT = getField(df, "SOUNDFONT")
SUB_CONFIG_FILE = getField(df, "SUB_CONFIG_FILE")

print(f".{SYNTH}.")
print(f".{SOUNDFONT}.")
print(f".{SUB_CONFIG_FILE}.")
...
class Tracks:
    def __init__(self, name, tracks):
        self.name = name
        self.tracks = []
        self.MyMIDI = MIDIFile(tracks, adjust_origin=True)
    ...
    def play(self, debug = False):
        '''
        create .mid and then run the file
        '''
        midiFile = f"{self.name}.mid"
        ...
        def useTimidity():
            ...
            for proc in psutil.process_iter():
                try:
                    if proc.name() == "timidity":
                        proc.kill()
                except:
                    pass    # supress errors with zombie processes
                
            #Timidity = "/usr/local/bin/timidity"
            Timidity = "timidity"
            # Config = "~/home/Soundfonts/timidity.cfg"
            cmd = f"{Timidity} --reverb=3 --quiet=9 -c {SUB_CONFIG_FILE} {midiFile}"   
            print(cmd)
        ...
        def useFluidSynth():
            ...
            # Soundfont = "/Users/seddon/home/Soundfonts/merlin_gold.sf2"
            cmd = f"{FluidSynth} -i {SOUNDFONT} {midiFile}"   
            print(cmd)
            subprocess.call(cmd.split())
        ...
        def z():
            ...
            SOUNDFONT = getField(df, "SOUNDFONT")
            SUB_CONFIG_FILE = getField(df, "SUB_CONFIG_FILE")
        if SYNTH == "fluidsynth": useFluidSynth()
        if SYNTH == "timidity": useTimidity()
        if debug: subprocess.call(["/usr/local/bin/midicsv", midiFile])
